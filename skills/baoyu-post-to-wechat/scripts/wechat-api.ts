...
async function main(): Promise<void> {
  ...
  // Apply EXTEND.md defaults for theme/color BEFORE rendering (account-level overrides global)
  const _earlyExtConfig = loadWechatExtendConfig();
  const _earlyResolved = resolveAccount(_earlyExtConfig, args.account);
  if (args.theme === "default" && _earlyResolved.default_theme) {
    args.theme = _earlyResolved.default_theme;
  }
  if (!args.color && _earlyResolved.default_color) {
    args.color = _earlyResolved.default_color;
  }
